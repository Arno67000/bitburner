...
export async function autoRun(ns) {
    ...
    targets.toLightHack.forEach((target) => ns.run("attack.js", 1, target, "lightHack.js"));
    targets.toGrow.forEach((target) => ns.run("distantRun.js", 1, target, "grow.js", 8));
    // targets.toUnlock.forEach((target) => ns.run("unlock.js", 1, target));

    targets.toHack.forEach((target) => ns.run("batcher.js", 1, target));
    // targets.toHack.forEach((target) => ns.run("launch.js", 1, target));
    // targets.toHack.forEach((target) => ns.run("batchRunner.js", 1, target));

    targets.toRemoteHack.forEach((target) => ns.run("remoteHack.js", 1, target, 80));

    // Logging attacked servers
    ns.write("autoLog.txt", `\r\n ${new Date().toLocaleTimeString()} : ${JSON.stringify(targetList)}`, "a");
    return;
}
...
function serverUnattackable(ns, server) {
    return (server.numOpenPortsRequired === 5 && !ns.fileExists("SQLInject.exe", "home")) ||
    (server.numOpenPortsRequired === 4 && !ns.fileExists("HTTPWorm.exe", "home")) ||
    (server.numOpenPortsRequired === 3 && !ns.fileExists("relaySMTP.exe", "home")) ||
    (server.numOpenPortsRequired === 2 && !ns.fileExists("FTPCrack.exe", "home")) ||
    (server.numOpenPortsRequired === 1 && !ns.fileExists("BruteSSH.exe", "home")) ||
    server.requiredHackingSkill > ns.getPlayer().hacking;
}

/** @param {Server} server*/
function serverUnlockable(server) {
    return (server.moneyAvailable === 0 && !server.hasAdminRights) || !server.backdoorInstalled;
}

/** @param {NS} ns @param {Server} server*/
function serverAlreadyHacked(ns, server) {
    return server.moneyAvailable === 0 || 
            (ns.getRunningScript("hack.js", "home", server.hostname, 0, server.hostname) !== null) || 
            server.ramUsed > 0;
}
...
function serverRemotelyHackable(ns, server) {
    return server.moneyAvailable > 0 &&
            (server.maxRam === 0 && !ns.getRunningScript("hack.js", "home", server.hostname, 0, server.hostname));
}